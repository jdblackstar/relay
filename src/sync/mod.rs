...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn scoped_prepublication_failure_cleans_temp_without_state_or_history() -> io::Result<()> {
        let (_tmp, cfg) = setup()?;
        let source = tempfile::TempDir::new()?;
        let selected_path = write_skill(source.path(), "selected", &doc("selected", "Selected"))?;
        let selected = discover_scoped_skills(std::slice::from_ref(&selected_path))?;
        let external = source.path().join("external");
        fs::create_dir_all(&external)?;
        std::os::unix::fs::symlink(&external, selected_path.join("linked-directory"))?;

        let err = sync_scoped_skills_with_mode(
            &cfg,
            &selected,
            LogMode::Quiet,
            ExecutionMode::Apply,
        ...
        assert!(!cfg.central_skills_dir.join("selected").exists());
        let leftovers = if cfg.central_skills_dir.exists() {
            fs::read_dir(&cfg.central_skills_dir)?
        ...
        assert!(leftovers
            .iter()
            .all(|name| !name.to_string_lossy().contains(".relay.tmp.")));
        assert!(!cfg.skill_state_path()?.exists());
        assert!(HistoryStore::from_config(&cfg)?.list_recent(5)?.is_empty());
        Ok(())
    }
    ...
    #[cfg(unix)]
    #[test]
    fn scoped_post_discovery_path_type_changes_abort_before_writes() -> io::Result<()> {
        ...
        for replacement in [
            Replacement::RootFile,
            Replacement::RootSymlink,
            Replacement::SkillDirectory,
            Replacement::SkillSymlink,
        ] {
            let (_tmp, cfg) = setup()?;
            let unrelated = write_skill(
                &cfg.central_skills_dir,
                "unrelated",
                &doc("unrelated", "Existing state"),
            )?;
            let baseline = discover_scoped_skills(std::slice::from_ref(&unrelated))?;
            sync_scoped_skills_with_mode(
                &cfg,
                &baseline,
                LogMode::Quiet,
            ...
            let first_before = fs::read(first.join("SKILL.md"))?;
            let selected = discover_scoped_skills(&[first.clone(), second.clone()])?;
            let outside_dir = source.path().join("outside-dir");
            let outside_file = source.path().join("outside-SKILL.md");
            fs::create_dir_all(&outside_dir)?;
            write_plain(&outside_file, &doc("second", "Outside"))?;

            match replacement {
                Replacement::RootFile => {
                    fs::remove_dir_all(&second)?;
                    fs::write(&second, "not a directory")?;
                }
                Replacement::RootSymlink => {
                    fs::remove_dir_all(&second)?;
                    symlink(&outside_dir, &second)?;
                }
                Replacement::SkillDirectory => {
                    fs::remove_file(second.join("SKILL.md"))?;
                    fs::create_dir(second.join("SKILL.md"))?;
                }
                Replacement::SkillSymlink => {
                    fs::remove_file(second.join("SKILL.md"))?;
                    symlink(&outside_file, second.join("SKILL.md"))?;
                }
            }
            ...
            assert!(
                err.to_string()
                    .contains("expected a regular skill directory")
                    || err.to_string().contains("expected a regular SKILL.md file")
            );
            assert_eq!(fs::read(first.join("SKILL.md"))?, first_before);
            for root in [
                &cfg.central_skills_dir,
                &cfg.claude_skills_dir,
                &cfg.codex_skills_dir,
    ...
    #[cfg(unix)]
    #[test]
    fn scoped_discovery_rejects_fifo_inside_package_before_any_write() -> io::Result<()> {
        use std::ffi::CString;
        use std::os::unix::ffi::OsStrExt;

        let (_tmp, cfg) = setup()?;
        let source = tempfile::TempDir::new()?;
        let valid = write_skill(source.path(), "a-valid", &doc("a-valid", "Valid"))?;
        let faulty = write_skill(source.path(), "z-faulty", &doc("z-faulty", "Faulty"))?;
        let fifo = faulty.join("assets/input.pipe");
        fifo.parent().map(fs::create_dir_all).transpose()?;
        let fifo = CString::new(fifo.as_os_str().as_bytes()).map_err(io::Error::other)?;
        // SAFETY: `fifo` is a live NUL-terminated path and `mkfifo` does not retain it.
        if unsafe { libc::mkfifo(fifo.as_ptr(), 0o600) } != 0 {
            return Err(io::Error::last_os_error());
        }

        let err = discover_scoped_skills(&[valid, faulty]).unwrap_err();

        assert_eq!(err.kind(), io::ErrorKind::InvalidInput);
        assert!(err
            .to_string()
            .contains("unsupported non-regular entry inside selected skill package"));
        for root in [
            &cfg.central_skills_dir,
            &cfg.claude_skills_dir,
            &cfg.codex_skills_dir,
            ...
            assert!(!root.join("z-faulty").exists());
        }
        assert!(!cfg.skill_state_path()?.exists());
        assert!(HistoryStore::from_config(&cfg)?.list_recent(5)?.is_empty());
        Ok(())
    ...
    #[cfg(unix)]
    #[test]
    fn scoped_preflight_rejects_invalid_existing_destination_roots_without_mutation(
    ) -> io::Result<()> {
        enum ExistingRoot {
            CanonicalSymlink,
            CanonicalMalformed,
        ...
        for case in [
            ExistingRoot::CanonicalSymlink,
            ExistingRoot::CanonicalMalformed,
            ExistingRoot::AdapterSymlink,
            ExistingRoot::AdapterMalformed,
        ] {
            let (_tmp, cfg) = setup()?;
            let source = tempfile::TempDir::new()?;
            let valid = write_skill(source.path(), "a-valid", &doc("a-valid", "Valid"))?;
            let faulty = write_skill(source.path(), "z-faulty", &doc("z-faulty", "Faulty"))?;
            let selected = discover_scoped_skills(&[valid, faulty])?;
            ...
            let destination = match case {
                ExistingRoot::CanonicalSymlink | ExistingRoot::CanonicalMalformed => {
                    cfg.central_skills_dir.join("z-faulty")
                }
                ExistingRoot::AdapterSymlink | ExistingRoot::AdapterMalformed => {
                    cfg.claude_skills_dir.join("z-faulty")
                }
            };
            match case {
                ExistingRoot::CanonicalSymlink | ExistingRoot::AdapterSymlink => {
                    destination.parent().map(fs::create_dir_all).transpose()?;
                    std::os::unix::fs::symlink(&outside, &destination)?;
                }
                ExistingRoot::CanonicalMalformed | ExistingRoot::AdapterMalformed => {
                    write_plain(&destination.join("sentinel.txt"), "malformed destination")?;
                }
            }
