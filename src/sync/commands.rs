use super::shared::{
    ...
    merge_frontmatter, read_markdown_variant, select_markdown_winner, update_markdown_target,
    write_file, MarkdownDoc, MarkdownVariant, RELAY_COMMAND_SKILL_MARKER, TOOL_CENTRAL,
};
...
pub(crate) fn sync_commands_with_mode(
    ...
    for name in names {
        ...
        for (tool, enabled, base_dir) in [
            ...
            if tool == TOOL_CODEX {
                let updated = sync_codex_command_skill(
                    source,
...
fn sync_codex_command_skill(
    ...
    if skill_dir.exists() && !marker_path.exists() {
        log_action(
            log_mode,
    ...
    let existing_skill = fs::read_to_string(&skill_path).ok();
    let existing_marker = fs::read(&marker_path).ok();
    if existing_skill.as_deref() == Some(contents.as_str())
        && existing_marker.as_deref() == Some(marker.as_slice())
    {
        return Ok(false);
    }
    ...
    write_file(&marker_path, marker, mode, history)?;
    write_file(&skill_path, contents.as_bytes(), mode, history)?;
