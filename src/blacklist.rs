...
fn command_basename(suffix: &Path) -> Option<String> {
...
mod tests {
    ...
    use crate::sync::test_support::{make_config, setup, write_plain, write_skill};
    ...
    #[test]
    fn resolve_tool_paths_includes_codex_command_skill_when_marker_present() {
        let tmp = TempDir::new().unwrap();
        let cfg = make_config(&tmp);
        let skill_dir = cfg.codex_skills_dir.join("review");
        std::fs::create_dir_all(&skill_dir).unwrap();
        write_plain(
            &skill_dir.join(RELAY_COMMAND_SKILL_MARKER),
    ...
    #[test]
    fn resolve_tool_paths_excludes_codex_command_skill_without_marker() {
        let tmp = TempDir::new().unwrap();
        let cfg = make_config(&tmp);
        write_skill(
    ...
    fn retroactive_delete_removes_codex_command_skill_wrapper() -> io::Result<()> {
        ...
        write_skill(
            &cfg.codex_skills_dir,
            "review",
            "---\nname: review\ndescription: Synced relay command.\n---\nWrapper body",
        )?;
        write_plain(
            &skill_dir.join(RELAY_COMMAND_SKILL_MARKER),
            "generated by relay from commands\n",
